...
typeMatch
    : primitiveType (LBRACK RBRACK)* // WARN syntactic predicates removed
    |	IDENTIFIER (typeArguments)? (DOT IDENTIFIER (typeArguments)? )* (LBRACK RBRACK)* // WARN syntactic predicates removed
    ;
...
expression returns [BaseDescr result]
    :	left=conditionalExpression { if( buildDescr  ) { $result = $left.result; } }
        (op=assignmentOperator right=expression)? // WARN syntactic predicates removed
    ;
